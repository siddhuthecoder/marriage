...
import { FiPlus, FiTrash2, FiEdit2, FiX, FiDollarSign, FiClock, FiCheck, FiAlertCircle } from 'react-icons/fi';
...
function App() {
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`/api/expenses/${currentExpense._id}/payments`, {
        amount,
  ...
  const fetchPaymentHistory = async (expenseId) => {
    try {
      const response = await axios.get(`/api/expenses/${expenseId}/payments`);
